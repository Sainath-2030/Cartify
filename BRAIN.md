...
| | **Phase 2.2: Home Page Redesign & Catalogue Quality Fix** | **COMPLETE** | Refined proportional typography and whitespace in the editorial hero with preserved Carbon Obsidian spotlight; redesigned dynamic department discovery with curated representative Unsplash photography and subcategory tags; fixed underlying catalogue data quality and domain classification in PostgreSQL (17,935 verified items; zero diapers in Beauty, zero luggage racks/toys in Fashion, zero generic cables in Gaming; deduplicated ASINs and cross-shelf repetition eliminated); full-width dark AI Recommendation gateway with honest telemetry notice and real community favorites; 4:5 luxury `ProductCard` with hover quick-add; session-based recently viewed shelf. |
