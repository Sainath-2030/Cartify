...
def top_k_similar(product_id: str, k: int = 10):
    normalized, id_map = load_index()
    if product_id not in id_map:
        raise ValueError(f"Unknown product_id: {product_id}")

    idx = id_map.index(product_id)
    ...
    results = []
    for i in ranked:
        if id_map[i] == product_id:
            continue
        results.append({"product_id": id_map[i], "score": float(scores[i])})
        if len(results) >= k:
            break
    return results
