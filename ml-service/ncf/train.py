...
def hit_rate_at_k(model, test_df, user_positive_items, num_items, k=10, n_candidates=99, device="cpu"):
    ...
    """
    if test_df.empty or num_items <= 1:
        return 0.0
    import numpy as np
    ...
    with torch.no_grad():
        for row in test_df.itertuples():
            ...
            unseen = [c for c in range(num_items) if c not in seen and c != row.item_idx]
            if unseen:
                num_cands = min(n_candidates, len(unseen))
                sampled_cands = rng.choice(unseen, size=num_cands, replace=False).tolist()
            else:
                sampled_cands = []
            candidates = [row.item_idx] + sampled_cands
