...
// Standard 8 Categories with high-aesthetic Unsplash photography representative of each department
const CATEGORIES = [
  {
    name: 'Electronics',
    slug: 'electronics',
...
// Curated Literature, Computer Science, Finance and Growth Library
const CURATED_BOOKS = [
  {
...
// -------------------------------------------------------------
// STRICT DOMAIN INTEGRITY & CONTRADICTION FILTERS
// -------------------------------------------------------------

function checkDepartmentContradictions(dept, name, file) {
  ...
  if (dept === 'beauty') {
    const prohibited = [
      ...
      'towel', 'toy', 'clothes', 'dress', 'shoe'
    ];
    if (prohibited.some((p) => lower.includes(p))) {
      return { invalid: true, reason: 'Nursery/Diaper/Non-beauty product in Beauty' };
    }
  }

  // 2. Fashion Violations
  if (dept === 'fashion') {
    const prohibited = [
    ...
    if (prohibited.some((p) => lower.includes(p))) {
      return { invalid: true, reason: 'Non-fashion / Furniture / Toy item in Fashion' };
    }
  }
  ...
  if (dept === 'gaming') {
    ...
    if (prohibited.some((p) => lower.includes(p))) {
      return { invalid: true, reason: 'Generic cable/adapter in Gaming' };
    }
  }

  // 4. Electronics - Headphones Subcategory Violations
  if (dept === 'electronics' && file === 'Headphones.csv') {
    ...
    if (isAdapter) {
      return { invalid: true, reason: 'Audio dongle / cable in Headphones subcategory' };
    }
  }

  // 5. Grocery Violations
  if (dept === 'grocery') {
    const prohibited = ['shirt', 'pant', 'shoe', 'cosmetics', 'shampoo', 'electronics', 'cable'];
    if (prohibited.some((p) => lower.includes(p))) {
      return { invalid: true, reason: 'Non-food product in Grocery' };
    }
  }
...
async function runFinalPipeline() {
  console.log('================================================================');
  console.log('   CARTIFY FINAL CATALOGUE INTEGRITY PIPELINE (PHASE 2.2-FINAL) ');
  console.log('================================================================\n');
  ...
  const slugCounts = {};
  let totalRawRecordsScanned = 0;

  try {
    // 1. REAPPLY CLEAN SCHEMA
    ...
    // 3. SEED 8 CATEGORIES WITH ACCURATE REPRESENTATIVE PHOTOGRAPHY
    ...
    // 4. STRATIFIED & CLEANED IMPORT ACROSS DOMAIN CSVS
    console.log('\n4. Preprocessing, quality scoring, and validating dataset...');
    ...
    for (const [dept, files] of Object.entries(DEPARTMENT_FILES)) {
      ...
      for (const { file, subcat } of files) {
        ...
        for await (const line of rl) {
          ...
          // 1. Basic title / image validity check
          if (!rawName || rawName.length < 8) {
            const reason = 'Title missing or too short';
            rejectedRecords.push({ rawName, dept, reason });
            rejectionStats[reason] = (rejectionStats[reason] || 0) + 1;
            continue;
          }

          if (isBadPlaceholderImage(rawImage)) {
            const reason = 'Invalid or placeholder image URL';
            rejectedRecords.push({ rawName, dept, reason });
            rejectionStats[reason] = (rejectionStats[reason] || 0) + 1;
            continue;
          }

          // 2. Strict Category Contradiction Gate
          const contradictionCheck = checkDepartmentContradictions(dept, rawName, file);
          if (contradictionCheck.invalid) {
            const reason = contradictionCheck.reason;
            rejectedRecords.push({ rawName, dept, reason });
            rejectionStats[reason] = (rejectionStats[reason] || 0) + 1;
            continue;
          }
          ...
          if (actualPrice === 0 && discountPrice === 0) {
            const reason = 'Missing both actual and discount prices';
            rejectedRecords.push({ rawName, dept, reason });
            rejectionStats[reason] = (rejectionStats[reason] || 0) + 1;
            continue;
          }
          ...
          if (discountPrice < 90) {
            const reason = 'Price below minimum retail threshold (₹90)';
            rejectedRecords.push({ rawName, dept, reason });
            rejectionStats[reason] = (rejectionStats[reason] || 0) + 1;
            continue;
          }
          ...
          if (seenAsins.has(asin)) {
            const reason = 'Duplicate ASIN';
            rejectedRecords.push({ rawName, dept, reason });
            rejectionStats[reason] = (rejectionStats[reason] || 0) + 1;
            continue;
          }

          // Fingerprint deduplication (strip spaces, symbols, lowercase)
          const fingerprint = rawName.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 45);
          if (seenFingerprints.has(fingerprint)) {
            const reason = 'Duplicate product title fingerprint';
            rejectedRecords.push({ rawName, dept, reason });
            rejectionStats[reason] = (rejectionStats[reason] || 0) + 1;
            continue;
          }

          // Image reuse check: allow variant sharing only within same department/brand, reject across disparate products
          if (seenImageUrls.has(rawImage)) {
            const prev = seenImageUrls.get(rawImage);
            if (prev.dept !== dept) {
              const reason = 'Suspicious image reuse across different departments';
              rejectedRecords.push({ rawName, dept, reason });
              rejectionStats[reason] = (rejectionStats[reason] || 0) + 1;
              continue;
            }
          }
          seenImageUrls.set(rawImage, { dept, name: rawName });
      ...
      console.log(`    ✓ Clean verified products for ${dept}: ${deptCount}`);
    }

    // 5. EXTRACT AUTHENTIC PRO GAMING HARDWARE
    console.log('\n  Extracting authentic Pro Gaming hardware...');
    ...
    if (fs.existsSync(amazonDumpCsv)) {
      ...
      for await (const line of rl) {
        ...
        // Strictly reject generic cables in Gaming
        if (lowerName.includes('hdmi cable') || lowerName.includes('vga') || lowerName.includes('rj45') || lowerName.includes('ethernet adapter')) {
          const reason = 'Generic cable in Gaming';
          rejectedRecords.push({ rawName, dept: 'gaming', reason });
          rejectionStats[reason] = (rejectionStats[reason] || 0) + 1;
          continue;
        }
        ...
        gamingCount++;
        if (gamingCount >= 400) break;
      }
      console.log(`    ✓ Clean verified Gaming gear: ${gamingCount}`);
    }

    // 6. SEED CURATED LITERATURE, TECH & FINANCE BOOKS
    console.log('\n  Seeding curated Literature, Computer Science & Finance Books...');
    ...
    console.log(`    ✓ Seeded curated Books: ${CURATED_BOOKS.length}`);

    // 7. EMIT PROCESSED ARTIFACTS
    console.log('\n5. Generating final catalogue artifacts (Clean CSV, Rejected CSV, Quality Report, QA Contact Sheet)...');

    // Clean CSV
    const cleanCsvPath = path.join(processedDir, 'amazon-products-final.csv');
    const cleanCsvHeader = 'source,source_id,name,category_slug,subcategory,brand,price,discount_percentage,final_price,rating,review_count,main_image\n';
    const cleanCsvRows = allProducts.map((p) =>
      `"${p.source}","${p.source_id}","${p.name.replace(/"/g, '""')}","${p.category_slug}","${p.subcategory}","${p.brand}",${p.price},${p.discount_percentage},${p.final_price},${p.rating},${p.review_count},"${p.main_image}"`
    ).join('\n');
    fs.writeFileSync(cleanCsvPath, cleanCsvHeader + cleanCsvRows, 'utf8');

    // Rejected CSV
    const rejectedCsvPath = path.join(processedDir, 'rejected-products.csv');
    const rejectedCsvHeader = 'name,department,reason\n';
    const rejectedCsvRows = rejectedRecords.map((r) =>
      `"${(r.rawName || '').replace(/"/g, '""')}","${r.dept || ''}","${(r.reason || '').replace(/"/g, '""')}"`
    ).join('\n');
    fs.writeFileSync(rejectedCsvPath, rejectedCsvHeader + rejectedCsvRows, 'utf8');

    // Quality Report JSON
    const reportJsonPath = path.join(processedDir, 'catalogue-quality-report.json');
    const qualityReport = {
      timestamp: new Date().toISOString(),
      rawRecordsScanned: totalRawRecordsScanned,
      finalCertifiedProducts: allProducts.length,
      rejectedRecordsCount: rejectedRecords.length,
      rejectionBreakdown: rejectionStats,
      categoryDistribution: {},
    };
    for (const p of allProducts) {
      qualityReport.categoryDistribution[p.category_slug] = (qualityReport.categoryDistribution[p.category_slug] || 0) + 1;
    }
    fs.writeFileSync(reportJsonPath, JSON.stringify(qualityReport, null, 2), 'utf8');

    // QA Sample Contact Sheet HTML (20 representative items per department = 160 items total)
    const qaHtmlPath = path.join(processedDir, 'qa_contact_sheet.html');
    const qaItemsByDept = {};
    for (const p of allProducts) {
      if (!qaItemsByDept[p.category_slug]) qaItemsByDept[p.category_slug] = [];
      if (qaItemsByDept[p.category_slug].length < 20) {
        qaItemsByDept[p.category_slug].push(p);
      }
    }

    let qaCardsHtml = '';
    for (const [dept, items] of Object.entries(qaItemsByDept)) {
      qaCardsHtml += `<h2 style="grid-column: 1 / -1; margin-top: 32px; border-bottom: 2px solid #333; padding-bottom: 8px; text-transform: uppercase;">${dept} (${items.length} Samples)</h2>`;
      for (const item of items) {
        qaCardsHtml += `
          <div style="border: 1px solid #ddd; border-radius: 8px; padding: 12px; background: #fff; display: flex; flex-direction: column; font-family: sans-serif;">
            <div style="height: 160px; display: flex; align-items: center; justify-content: center; background: #f8f8f8; border-radius: 6px; overflow: hidden; margin-bottom: 8px;">
              <img src="${item.main_image}" alt="${item.name}" style="max-height: 100%; max-width: 100%; object-fit: contain;" />
            </div>
            <span style="font-size: 10px; font-weight: bold; color: #888; text-transform: uppercase;">${item.brand} • ${item.subcategory}</span>
            <h4 style="font-size: 12px; margin: 4px 0 8px; line-height: 1.3; height: 32px; overflow: hidden;">${item.name}</h4>
            <div style="margin-top: auto; display: flex; justify-content: space-between; font-size: 12px; font-weight: bold;">
    ...
    const qaHtml = `
      ...
      <html>
      <head>
        <title>Cartify Catalogue QA Contact Sheet</title>
        <meta charset="utf-8" />
        <style>
          body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background: #f4f4f5; padding: 24px; color: #18181b; }
          .grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 16px; }
        </style>
      </head>
      <body>
        <h1>Cartify Catalogue QA Contact Sheet (160 Representative Items)</h1>
        <p>Generated on ${new Date().toLocaleString()} | Total Certified Products: ${allProducts.length}</p>
        <div class="grid">${qaCardsHtml}</div>
      </body>
      </html>
    `;
    fs.writeFileSync(qaHtmlPath, qaHtml, 'utf8');
    console.log('✓ Artifacts emitted successfully in data/processed/.');

    // 8. BATCH INSERTION INTO POSTGRESQL
    console.log(`\n6. Batch inserting ${allProducts.length} certified products into PostgreSQL...`);
    ...
    for (let i = 0; i < allProducts.length; i += BATCH_SIZE) {
      ...
      for (const p of batch) {
        ...
        params.push(
          ...
          p.seller_name, p.main_image, p.images
        );
      }
      ...
      const insertSql = `
        INSERT INTO products (
          ...
          seller_name, main_image, images
        ) VALUES ${valueSets.join(', ')}
      `;

      await client.query(insertSql, params);
      process.stdout.write(`  Inserted ${Math.min(i + BATCH_SIZE, allProducts.length)} / ${allProducts.length} products...\r`);
    }

    await client.query('COMMIT');
    console.log(`\n✓ All ${allProducts.length} certified products inserted into PostgreSQL successfully!`);
    ...
    console.log('\n8. Final certified database distribution:');
    ...
    console.error('Final pipeline error:', err);
    throw err;
  } finally {
    client.release();
    await pool.end();
