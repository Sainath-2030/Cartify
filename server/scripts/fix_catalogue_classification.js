...
// Rich Curated Library of 50 Classic, Tech, Business, and Fiction Books
const CURATED_BOOKS = [
  {
...
// -------------------------------------------------------------
// STRICT DOMAIN INTEGRITY FILTERS
// -------------------------------------------------------------

function isInvalidForDepartment(dept, name, file) {
  const lower = name.toLowerCase();

  // Beauty must strictly NOT contain baby diapers, pampers, baby bath accessories, straw cups, aligners
  if (dept === 'beauty') {
    const prohibited = [
      ...
      'maternity', 'rash cream', 'ointment', 'baby care', 'sipper', 'feeding bottle', 'crib'
    ];
    if (prohibited.some((p) => lower.includes(p))) return true;
  }

  // Fashion must strictly NOT contain furniture, racks, toys, medical retainer cases, gift keychains
  if (dept === 'fashion') {
    const prohibited = [
      ...
      'tableware', 'drying stand', 'organizer box', 'medicine', 'keychain'
    ];
    if (prohibited.some((p) => lower.includes(p))) return true;
  }

  // Headphones must NOT be audio adapters, converters, or phone charging cables
...
async function runClassificationFix() {
  console.log('================================================================');
  console.log('   CARTIFY CATALOGUE RECLASSIFICATION & PURITY PIPELINE        ');
  console.log('================================================================\n');
  ...
  try {
    ...
    // 3. SEED 8 CATEGORIES WITH ACCURATE EDITORIAL IMAGERY
    console.log('\n3. Seeding 8 top-level standard categories with representative imagery...');
    const catMap = {};
    ...
    // 4. STRATIFIED & CLEANED IMPORT
    console.log('\n4. Preprocessing and importing certified products with strict domain filters...');
    const allProducts = [];
    ...
    for (const [dept, files] of Object.entries(DEPARTMENT_FILES)) {
      ...
      console.log(`\n  Processing [${dept.toUpperCase()}] (${files.length} sources)...`);
      ...
      for (const { file, subcat } of files) {
        ...
        for await (const line of rl) {
          ...
          if (isInvalidForDepartment(dept, rawName, file)) continue;
          ...
          const dedupKey = rawName.toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 45);
    ...
    // 5. EXTRACT HIGH-PRECISION GAMING GEAR
    console.log('\n  Extracting authentic Pro Gaming gear...');
    ...
    if (fs.existsSync(amazonDumpCsv)) {
      ...
      for await (const line of rl) {
        ...
        // Prohibited generic cables/adapters in Gaming
    ...
    // 6. SEED CURATED BOOKS
    console.log('\n  Seeding curated Bestseller Literature & Tech Books...');
    ...
    console.log(`\n================================================================`);
    console.log(`   TOTAL CERTIFIED PRODUCTS READY TO SEED: ${allProducts.length}`);
    console.log(`================================================================\n`);

    // 7. BATCH INSERTION IN TRANSACTION
    console.log('5. Inserting certified records into PostgreSQL...');
    ...
    console.log(`\n✓ All ${allProducts.length} certified products inserted successfully!`);

    // 8. UPDATE SEARCH VECTORS
    console.log('\n6. Updating PostgreSQL full-text search vectors...');
    await client.query(`
    ...
    console.log('\n7. Final verified distribution across all 8 departments:');
...
runClassificationFix().catch(console.error);
